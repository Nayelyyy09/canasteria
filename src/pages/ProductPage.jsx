      ...
      <>
        ...
        <div className="min-h-screen flex items-center justify-center bg-[#FAF7F2]">
          <div className="text-center px-6">
            <h1 className="font-display text-4xl text-[#0A461A] mb-4">Producto no encontrado</h1>
            <p className="font-body text-[#0A461A]/60 mb-8">
    ...
    <>
      ...
      <main className="bg-[#FAF7F2] min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-16 py-4">
          <nav className="flex items-center gap-2 text-sm font-body text-[#0A461A]/50">
            <Link to="/" className="hover:text-[#7E0E0F] transition-colors">
              Inicio
            </Link>
            <ChevronRight size={14} />
            <Link
              to={`/canastas-navidenas/${product.category}`}
              className="hover:text-[#7E0E0F] transition-colors"
            >
              {category?.label || product.category}
            </Link>
            <ChevronRight size={14} />
            <span className="text-[#0A461A]">{product.name}</span>
          </nav>
        </div>
        ...
        <section className="max-w-7xl mx-auto px-6 lg:px-16 pb-16">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
            ...
            >
              <span className="inline-flex items-center gap-2 px-3 py-1 bg-[#7E0E0F]/10 text-[#7E0E0F] text-xs tracking-widest uppercase font-body w-fit mb-4">
                {category?.label || product.category}
              </span>

              <h1 className="font-display text-3xl sm:text-4xl lg:text-5xl text-[#0A461A] leading-tight">
                {product.name}
              </h1>

              <div className="mt-4 w-16 h-px bg-[#C9A96E]" />

              <p className="mt-6 font-body text-[#0A461A]/70 leading-relaxed text-base lg:text-lg">
                {product.description}
              </p>

              {/* Contents */}
              <div className="mt-8">
                <h2 className="font-display text-xl text-[#0A461A] mb-4">Contenido de la canasta</h2>
                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {product.contents.map((item, i) => (
                    <li key={i} className="flex items-start gap-2 font-body text-sm text-[#0A461A]/70">
        ...
        {relatedProducts.length > 0 && (
          <section className="max-w-7xl mx-auto px-6 lg:px-16 pb-20">
            <div className="border-t border-[#C9A96E]/20 pt-12">
              <h2 className="font-display text-2xl lg:text-3xl text-[#0A461A] mb-8">
