    ...
    >
      <Link to={`/producto/${product.slug}`} className="block">
        <div className="relative aspect-[3/4] overflow-hidden mb-5">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-[#1A2F23]/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
          ...
          >
            ...
            <motion.button
              ...
              className="w-12 h-12 bg-[#841B2D] text-[#F9F4EB] rounded-full flex items-center justify-center hover:bg-[#6d1625] transition-colors shadow-lg hover:scale-110 duration-300"
            >
              <Eye size={20} />
            </motion.button>
          </motion.div>
        ...
        <div className="px-1">
          <div className="flex items-start justify-between gap-4">
            <h3 className={`font-display text-xl lg:text-2xl leading-tight group-hover:text-[#841B2D] transition-colors duration-300 ${dark ? 'text-[#F9F4EB]' : 'text-[#1A2F23]'}`}>
              {product.name}
            </h3>
          </div>
          <motion.div
